...
```bash
curl -fsSL https://your-domain.com/install-openclaw-easy.sh | bash
```

### 方式二：下載可執行檔

1. 從 [Releases](https://github.com/yourname/openclaw-easy-installer/releases) 下載對應平台版本
2. 賦予執行權限：`chmod +x openclaw-installer`
3. 執行：`./openclaw-installer`

### 方式三：Python 腳本

```bash
# 克隆專案
git clone https://github.com/yourname/openclaw-easy-installer.git
...
### Q: 支援哪些平台？
A: 目前支援 macOS、Linux（主流發行版）、Windows WSL2

### Q: 如何更新 OpenClaw？
A: 運行 `npm update -g openclaw` 或直接重新運行安裝器

### Q: 這是官方工具嗎？
A: 不是，這是社群製作的第三方安裝輔助工具
